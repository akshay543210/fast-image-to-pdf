...
export function DropZone({ onFiles, disabled = false, compact = false }: DropZoneProps) {
  ...
  return (
    <button
      ...
      className={cn(
        "group flex w-full cursor-pointer flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-ink/50 bg-card text-center transition-all",
    ...
    >
      ...
      {!compact && (
        <span className="text-sm text-muted-foreground">
          or <span className="font-semibold text-primary underline underline-offset-2">browse your files</span> — pasting works too
        </span>
      )}
      <span id="dropzone-hint" className="font-mono text-xs uppercase tracking-wider text-muted-foreground">
